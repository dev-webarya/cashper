    ...
    switch(step) {
      ...
      case 2:
        if (!formData.loanAmount || formData.loanAmount < 100000 || formData.loanAmount > 10000000) {
          alert('Please enter a valid loan amount between ₹1,00,000 and ₹1,00,00,000');
          return false;
        }
        if (!formData.purpose) {
        ...
        if (!formData.monthlyIncome || formData.monthlyIncome < 25000) {
          alert('Please enter a valid monthly income (minimum ₹25,000)');
          return false;
        }
        return true;
      case 3:
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateStep(4) && !isTransitioning) {
      ...
      try {
        ...
        const response = await submitLoanApplication('business-loan', formData, documents);
