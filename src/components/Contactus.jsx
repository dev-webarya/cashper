...
---
This message was sent from the Cashper.ai Contact Form.
      `;

      // Send email using mailto (opens user's email client)
      const mailtoLink = `mailto:info@cashper.ai?subject=${encodeURIComponent(`Contact Form: ${formData.subject}`)}&body=${encodeURIComponent(emailBody)}`;
      ...
      setTimeout(() => {
        setSuccessMessage("Thank you! Your email client has been opened. Please send the email to complete your inquiry.");
        setFormData({ name: "", email: "", phone: "", subject: "", message: "" });
        setIsSubmitting(false);
      }, 500);
    } catch (error) {
      setErrors({ submit: "Failed to open email client. Please try again or call us directly at 6200755759<br/>7393080847." });
      setIsSubmitting(false);
    }
  };
