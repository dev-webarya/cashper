...
const Aboutus = () => {
  ...
  // Data arrays - moved to top to prevent initialization errors
  const testimonials = [
    {
      name: "Rajesh Kumar",
  ...
  const stats = [
    ...
    { icon: TrendingUp, value: "₹5000 Cr", label: "Loans Disbursed", color: "text-emerald-600" },
    { icon: Star, value: "4.9/5", label: "Customer Rating", color: "text-yellow-500" },
    { icon: Clock, value: "2 hrs", label: "Avg. Approval Time", color: "text-teal-600" },
    { icon: Building2, value: "100+", label: "Partner Banks & NBFCs", color: "text-green-700" },
    { icon: Globe, value: "100+", label: "Cities Served", color: "text-emerald-700" },
  ...
  const leadership = [
    ...
    },
    {
      ...
      image: "/team/Awdhesh kumar.jpg"
    },
    {
      name: "Saurav Raunak",
      position: "Store Manager at Blinkit",
      image: "/team/Saurav Raunak.jpg"
    }
  ];
  return (
    <motion.div
    ...
    >
      ...
      {/* Leadership Team - Simple Grid Layout */}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 xs:gap-5 sm:gap-6 md:gap-8">
            {leadership.map((leader, index) => (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.6, delay: index * 0.1 }}
                className="bg-white rounded-lg sm:rounded-xl md:rounded-2xl p-4 xs:p-5 sm:p-6 shadow-lg hover:shadow-xl transition-all duration-300 text-center group hover:-translate-y-1"
              >
                <div className="w-20 h-20 xs:w-24 xs:h-24 sm:w-28 sm:h-28 md:w-32 md:h-32 bg-gradient-to-br from-green-100 to-emerald-100 rounded-full mx-auto mb-4 xs:mb-5 sm:mb-6 flex items-center justify-center overflow-hidden">
                  <img 
                    src={leader.image} 
                    alt={leader.name}
                ...
                <h3 className="text-base xs:text-lg sm:text-xl font-bold text-gray-900 mb-1.5 xs:mb-2">{leader.name}</h3>
                <p className="text-green-600 font-medium mb-2.5 xs:mb-3 text-sm xs:text-base">{leader.position}</p>
                ...
                <div className="space-y-1.5 xs:space-y-2 text-xs xs:text-sm text-gray-600">
                  ...
                  {leader.education && (
                    <div className="flex items-center justify-center gap-1.5 xs:gap-2">
                      <GraduationCap className="w-3.5 h-3.5 xs:w-4 xs:h-4 flex-shrink-0" />
