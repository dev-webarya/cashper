...
const Settings = () => {
  ...
  const loginHistory = [
    { device: 'Windows PC - Chrome', location: 'Delhi, India', time: '2 hours ago', status: 'current' },
    { device: 'Android App', location: 'Delhi, India', time: 'Yesterday at 3:45 PM', status: 'past' },
    { device: 'iPhone - Safari', location: 'Mumbai, India', time: '3 days ago', status: 'past' },
  ];
  ...
  const handleToggleNotification = (type) => {
    switch(type) {
      ...
      case 'push':
        ...
        setTimeout(() => {
          alert(`Push notifications ${!pushNotifications ? 'enabled' : 'disabled'} successfully!`);
        }, 100);
        break;
    }
  };
      ...
      {showDownloadData && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 relative animate-fadeInUp">
            ...
            <button 
              onClick={() => {
                alert('Data download requested!\n\nYour data will be prepared and sent to your registered email (john.doe@example.com) within 24-48 hours.');
