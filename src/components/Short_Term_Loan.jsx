    ...
    switch(step) {
      case 1:
        ...
        if (!formData.relativeName.trim()) {
          alert('Please enter relative/reference name');
          return false;
        }
        ...
        if (!formData.relativePhone.trim() || !/^[0-9]{10}$/.test(formData.relativePhone.replace(/\D/g, ''))) {
          alert('Please enter a valid 10-digit relative/reference phone number');
          return false;
        }
        return true;
      case 2:
        if (!formData.loanAmount || formData.loanAmount < 10000 || formData.loanAmount > 500000) {
          alert('Please enter a valid loan amount between ₹10,000 and ₹5,00,000');
          return false;
        }
        if (!formData.purpose) {
        ...
        if (!formData.monthlyIncome || formData.monthlyIncome < 15000) {
          alert('Please enter a valid monthly income (minimum ₹15,000)');
          return false;
        }
        return true;
      case 3:
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateStep(4) && !isTransitioning) {
      ...
      try {
        ...
        const response = await submitLoanApplication('short-term-loan', formData, documents);
        
        console.log('Application submitted successfully:', response);
        
        // Store reference number for success screen
