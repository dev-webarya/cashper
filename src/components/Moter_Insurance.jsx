  ...
  const validateStep = (step) => {
    switch(step) {
      case 1:
        ...
        if (!formData.manufacturingYear || formData.manufacturingYear < 1990 || formData.manufacturingYear > new Date().getFullYear()) {
          alert('Please enter valid manufacturing year');
          return false;
        }
        if (!formData.vehicleValue) {
          alert('Please select vehicle value');
          return false;
        }
        return true;
      case 2:
        ...
        if (!formData.age || formData.age < 18 || formData.age > 75) {
          alert('Please enter a valid age (18-75 years)');
          return false;
        }
      ...
      case 3:
        ...
        return true;
      case 4:
        const requiredDocs = ['rc', 'dl', 'vehiclePhotos'];
        for (const doc of requiredDocs) {
          if (!documents[doc]) {
            alert(`Please upload ${doc === 'rc' ? 'Vehicle RC (Registration)' : doc === 'dl' ? 'Driving License' : 'Vehicle Photos'}`);
            return false;
          }
        }
        return true;
